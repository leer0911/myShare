...
Note:
早期的游戏是没有游戏引擎这个概念的，都是一个游戏一套代码，后面才把这些重用代码封装起来，成为了早期的引擎。它的任务就是尽量不用负责底层处理，而专心到游戏逻辑上来。
...
Note:
好比我们使用 ORM 框架来操作数据库，不用关心底层细节。对于前端开发者来说，游戏引擎应该支持 JavaScript ，并且可以用 Js 去调用底层接口。
...
Note:
就像前端的雪碧图，但与雪碧图不一样的是。ccc 中的图集由 plist 和 png 组成。
--
